...
ControlFlowGraph::ControlFlowGraph(const ProgramDesc& prog, const int block_id)
    : is_ir_graph(false) {
  for (OpDesc* op : prog.Block(static_cast<size_t>(block_id)).AllOps()) {
    std::unique_ptr<ir::Node> ir_node = ir::CreateDummyNode(op);
    ops_.push_back(std::move(ir_node.release()));
  }
}
...
void ControlFlowGraph::UpdateCFGGraph(const std::string& old_node,
